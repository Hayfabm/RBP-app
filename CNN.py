    ...
    class CNN(nn.Module):
        def __init__(self):
            ...
            self.conv1 = nn.Conv1d(
                in_channels=1, out_channels=64, kernel_size=5, stride=1, padding=0,
            )
            self.dropout = nn.Dropout(p=0.1)
            self.conv2 = nn.Conv1d(
                in_channels=64, out_channels=32, kernel_size=5, stride=1, padding=0,
            )

            self.fc1 = nn.Linear(32 * 608, num_classes)
